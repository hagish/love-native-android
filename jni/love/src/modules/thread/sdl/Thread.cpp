...
namespace sdl
{
	int threadfunc(ThreadData *comm)
	{
		...
		{
			pthread_mutex_lock((pthread_mutex_t*)comm->mutex);
			ThreadVariant *v = new ThreadVariant(lua_tostring(L, -1), lua_strlen(L, -1));
			comm->setValue("error", v);
			v->release();
			pthread_mutex_unlock((pthread_mutex_t*)comm->mutex);
			pthread_cond_broadcast((pthread_cond_t*)comm->cond);
		}
		lua_close(L);
		return 0;
	}
	...
	void Thread::start()
	{
		if (!handle && isThread)
		{
			...
			pthread_create(handle, NULL, (void* (*)(void*))threadfunc, (void*)comm);
		}
	}
	...
	void Thread::kill()
	{
		if (handle)
		{
			pthread_mutex_lock((pthread_mutex_t*)_gcmutex);
			pthread_kill(*handle, 9); // SDL_KillThread?
			handle = 0;
			pthread_mutex_unlock((pthread_mutex_t*)_gcmutex);
		}
	}
