...
namespace opengles
{
	...
	PixelEffect::PixelEffect(const std::string& vertex, const std::string& fragment)
		: _program(0), _vertex(vertex), _fragment(fragment)
	{
	...
	bool PixelEffect::loadVolatile()
	{
		...
		src = _fragment.c_str();
		strlen = _fragment.length();
		glShaderSource(fshader, 1, (const GLchar**)&src, &strlen);
		...
		{
			// get compiler error
			glGetShaderiv(fshader, GL_INFO_LOG_LENGTH, &strlen);
			char *error_str = new char[strlen];
	...
	{
		// we support OpenGL ES 2.0 only => shaders are mandatory
		return true;
	}
