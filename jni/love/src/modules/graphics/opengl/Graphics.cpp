...
//		}
	...
	Graphics::BlendMode Graphics::getBlendMode ()
	{
		...
		glGetIntegerv(GL_BLEND_EQUATION_OES, &equation);

		if (equation == GL_FUNC_REVERSE_SUBTRACT_OES) // && src == GL_SRC_ALPHA && dst == GL_ONE
...
//		return 2;
	...
	**/
	...
	void Graphics::point( float x, float y )
	{
		...
		glVertexPointer(2, GL_FLOAT, 0, (GLvoid*)vertex);
	...
	int Graphics::polyline( lua_State * L)
	{
		...
		if (table) {
			for (int i = 1; i < args; i += 2) {
				...
				lua_rawget(L, 1);
				data[i-1] = (GLfloat)lua_tonumber(L, -2);
				data[i] = (GLfloat)lua_tonumber(L, -1);
		...
		glEnableClientState(GL_VERTEX_ARRAY);
		glVertexPointer(2, GL_FLOAT, 0, (GLvoid*)data);
	...
	void Graphics::triangle(DrawMode mode, float x1, float y1, float x2, float y2, float x3, float y3 )
	{
		...
		float vertex[] = {x1, y1, x2, y2, x3, y3};
		switch(mode)
		{
		case DRAW_LINE:
			glEnableClientState(GL_VERTEX_ARRAY);
	...
	void Graphics::rectangle(DrawMode mode, float x, float y, float w, float h)
	{
		...
		case DRAW_LINE:
		{
			...
			glEnableClientState(GL_VERTEX_ARRAY);
			glVertexPointer(2, GL_FLOAT, 0, (GLvoid*)point);
	...
	void Graphics::circle(DrawMode mode, float x, float y, float radius, int points )
	{
		...
		float *data = new float[2*points];
		int coords = 0;
		for(float i = 0; i < two_pi; i+= angle_shift)
		{
			data[coords++] = radius * sin(i);
			data[coords++] = radius * cos(i);
		}
		...
		case DRAW_LINE:
			glEnableClientState(GL_VERTEX_ARRAY);
			glVertexPointer(2, GL_FLOAT, 0, (GLvoid*)data);
			glDrawArrays(GL_LINE_LOOP, 0, coords/2);
			glDisableClientState(GL_VERTEX_ARRAY);
			break;
		...
		case DRAW_FILL:
			glEnableClientState(GL_VERTEX_ARRAY);
			glVertexPointer(2, GL_FLOAT, 0, (GLvoid*)data);
			glDrawArrays(GL_TRIANGLE_FAN, 0, coords/2);
			glDisableClientState(GL_VERTEX_ARRAY);
			break;
		}
	...
	int Graphics::polygon( lua_State * L )
	{
		...
		// TODO: Gucken ob es funktioniert
		// GL_POLYGON als GL_TRIANGLE_STRIP?!?
		...
		case LUA_TNUMBER:
			for(int i = 2; i<n; i+=2)
			{
				vertices.push_back((GLfloat)lua_tonumber(L, i));
				vertices.push_back((GLfloat)lua_tonumber(L, i+1));
			}
			break;
		case LUA_TTABLE:
			lua_pushnil(L);
			while (true)
			{
				...
				vertices.push_back((GLfloat)lua_tonumber(L, -1));
		...
		GLfloat *fvertices = new GLfloat[vertices.size()];
		for(int i = 0; i < vertices.size(); ++i)
			fvertices[i] = vertices.at(i);


		glEnableClientState(GL_VERTEX_ARRAY);
		glVertexPointer(2, GL_FLOAT, 0, (GLvoid*)fvertices);
		glDrawArrays((mode==DRAW_LINE) ? GL_LINE_LOOP : GL_TRIANGLE_STRIP /*GL_POLYGON*/, 0, vertices.size()/2);
		glDisableClientState(GL_VERTEX_ARRAY);
